    ...
    def retrieve(
        ...
        for source_name, results in (("dense", dense_results), ("sparse", sparse_results)):
            for item in results:
                ...
                if existing is None:
                    merged = dict(item)
                    merged["dense_score"] = item.get("score", 0.0) if source_name == "dense" else 0.0
                    merged["sparse_score"] = item.get("score", 0.0) if source_name == "sparse" else 0.0
                    merged["score"] = float(item.get("score", 0.0))
                    combined[key] = merged
                else:
                    if source_name == "dense":
                        existing["dense_score"] = max(existing.get("dense_score", 0.0), item.get("score", 0.0))
                    else:
                        existing["sparse_score"] = max(existing.get("sparse_score", 0.0), item.get("score", 0.0))

                    existing["score"] = (
                        0.6 * float(existing.get("dense_score", 0.0))
                        + 0.4 * float(existing.get("sparse_score", 0.0))
                    )
