...
class FeedbackRepository:
    ...
    async def create(self, feedback_data: Any, db: AsyncSession) -> models.Feedback:
        """Create and persist user feedback."""
    ...
    async def get_user_feedback_for_query(
        self,
        query_id: int,
        db: AsyncSession,
    ) -> List[models.Feedback]:
        """Retrieve all feedback associated with a given query ID."""
        result = await db.execute(
            select(models.Feedback).where(models.Feedback.query_id == query_id)
        )
