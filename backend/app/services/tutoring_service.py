...
async def process_chat_query(
    ...
    target_concept = await route_concept_async(
        query_text,
    ...
    synthesized_answer = await synthesize_answer_async(
        query_text,
        evidence_dicts,
        user_config=user_config,
    )
    verification_result = await verify_answer_async(
        synthesized_answer,
        evidence_dicts,
        user_config=user_config,
    )
    ...
    if final_answer_text and evidence_dicts:
        claim_check_result = await check_claim_async(
            final_answer_text,
            evidence_dicts,
            user_config=user_config,
        )
